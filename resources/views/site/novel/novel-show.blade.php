...
@section('content')
    <div class="novel-hero">
        <div class="novel-hero__bg"
             style="background-image: url('{{ $novel->getCoverUrl() ?? asset('assets/images/img.png') }}')">
        </div>
        <div class="novel-hero__inner">
            {{-- Kapak --}}
            <div class="novel-hero__cover">
                <img src="{{ $novel->getCoverUrl() ?? asset('assets/images/img.png') }}"
                     alt="{{ $novel->title }}"
                ...
                <a class="novel-hero__start-btn"
                   href="{{ route('site.novel.chapter', ['novel' => $novel, 'chapter' => $firstChapter->chapter_number]) }}">
                ...
                @if($chapters->count() > 1)
                    <a class="novel-hero__start-btn novel-hero__start-btn--last"
                       href="{{ route('site.novel.chapter', ['novel' => $novel, 'chapter' => $lastChapter->chapter_number]) }}">
                        <span><img src="{{ asset('assets/icons/book-solid-full.svg') }}" alt="" style="width:100%;height:100%;max-width:20px"></span>
                        Son Bölümü Oku
                    </a>
                @endif
                ...
                @auth
                    <button class="novel-hero__follow-btn {{ $isFollowing ? 'novel-hero__follow-btn--active' : '' }}"
                            id="followBtn"
                            data-url="{{ route('follow.toggle', ['type' => 'novel', 'id' => $novel->id]) }}"
                            data-check-icon="{{ asset('assets/icons/check-solid-full.svg') }}"
                            data-plus-icon="{{ asset('assets/icons/plus-solid-full.svg') }}">
                        <span>
                            <img src="{{ $isFollowing ? asset('assets/icons/check-solid-full.svg') : asset('assets/icons/plus-solid-full.svg') }}"  alt="" style="width:100%; height:100%; max-width:20px">
                        </span>
                ...
                @else
                    <a href="javascript:;" data-open="#login-form" class="novel-hero__follow-btn">
                        <span>
                            <img src="{{ asset('assets/icons/plus-solid-full.svg') }}" alt="" style="width:100%; height:100%; max-width:20px">
                        </span> Takip Et
                    </a>
                @endauth
            </div>
            ...
            <div class="novel-hero__info">
                <h1 class="novel-hero__title">{{ $novel->title }}</h1>
                ...
                <span class="novel-meta-pill novel-meta-pill--chapters">
                    ...
                    @if($novel->scraped_at)
                        ...
                        {{ $novel->scraped_at->diffForHumans() }}
                    </span>
                    @endif
                    <span class="novel-meta-pill novel-meta-pill--status">
                        <span>
                            <img src="{{ asset('assets/icons/readme-brands-solid-full.svg') }}" alt="" style="width:100%; height:100%; max-width:15px">
                        </span> Novel
                    </span>
                    <span class="novel-meta-pill novel-meta-pill--status">
                        <span>
                ...
                <div class="novel-hero__rating">
                    <div class="novel-stars" id="novelStars"
                         data-url="{{ route('rate', ['type' => 'novel', 'id' => $novel->id]) }}"
                    ...
                    <div class="novel-desc-content">
                        ...
                        @else
                            <p class="novel-desc-placeholder">
                                ...
                                Bu novel için henüz bir açıklama eklenmemiş.
                            </p>
                        @endif
                    </div>
                </div>
                ...
                @guest
                    <div class="novel-auth-notice">
                        <span>
                            <img src="{{ asset('assets/icons/lock-solid-full.svg') }}" alt="" style="width:100%; height:100%; max-width:20px">
                        </span>
                        <span>
                        Takip etmek için
                        <a href="javascript:;" data-open="#login-form">giriş yapın</a>.
                    </span>
                    </div>
                @endguest
                        ...
                        <div class="reading-progress">
                            ...
                            <div class="reading-progress__footer">
                                ...
                                <a class="reading-progress__continue-btn"
                                   href="{{ route('site.novel.chapter', ['novel' => $novel, 'chapter' => $readingProgress]) }}">
    ...
    <div class="novel-section" style="margin-top: 20px;">
        ...
        <ul class="chapter-list" id="chapterList">
            @foreach($sortedChapters as $ch)
                @php
                    $isNew         = optional($novel->scraped_at)->gt(now()->subDay()) && optional($ch->created_at)->gt(now()->subDay());
                ...
                <li class="chapter-list__item {{ $loop->index >= 20 ? 'chapter-list__item--hidden' : '' }} {{ $isLastRead ? 'chapter-list__item--last-read' : '' }}">
                    <a href="{{ route('site.novel.chapter', ['novel' => $novel, 'chapter' => $ch->chapter_number]) }}">
...
@push('scripts')
    <script>
        (function () {
            ...
            var url        = starsWrap.dataset.url;
            var isGuest    = starsWrap.dataset.guest === '1';
            ...
            if (followBtn) {
                followBtn.addEventListener('click', function () {
                    ...
                    var checkIcon = btn.dataset.checkIcon;
                    var plusIcon  = btn.dataset.plusIcon;
                    btn.disabled = true;
                    fetch(url, {
                        method: 'POST',
                        headers: {
                        ...
                        .then(function (data) {
                            if (data.following) {
                                btn.classList.add('novel-hero__follow-btn--active');
                                icon.src = checkIcon;
                                text.textContent = 'Takip Ediliyor';
                            } else {
                                btn.classList.remove('novel-hero__follow-btn--active');
                                icon.src = plusIcon;
